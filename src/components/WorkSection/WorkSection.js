...
const WorkSection = (props) => {
    const { title, img, link, gh, skill } = props.work
    console.log(link)
        ...
        <div className="workContent col-sm-4">
            ...
            <div style={{ width: '100%' }} className="workContentDetails">
                ...
                <div className="useLanguageAndTools">
                    {
                        skill.map( txt => <span className="skillNameBlock">{txt}</span> )
                    }
                </div><br />
                {
                    gh.length >= 10 && <a title='Github repository' target='blank' href={gh} className="workViewGithubBtn text-danger">
                ...
                {
                    link.length >= 10 && <a target='blank' href={link} className="workViewWebsiteBtn text-danger">
